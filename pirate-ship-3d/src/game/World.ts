...
function buildIsland(radius: number, isHomePort: boolean): THREE.Group {
  ...
  const baseGeo = new THREE.ConeGeometry(radius, height, 20);
  ...
  const capGeo = new THREE.ConeGeometry(radius * 0.55, height * 0.5, 20);
  const capMat = new THREE.MeshStandardMaterial({ color: 0x3f7d3a, roughness: 0.95, map: specklTexture() });
  ...
  if (isHomePort) {
    ...
    for (let i = 0; i < 3; i++) {
      const palmTrunkGeo = new THREE.CylinderGeometry(0.08, 0.12, 1.8, 12);
      ...
      const leavesGeo = new THREE.SphereGeometry(0.5, 10, 8);
